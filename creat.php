...
class TableClassGenerator {
	...
	private function generateClassForTable($table) {
		$class = $this->classPre.ucfirst($this->transform($table));
		$class_factory = $class.'Factory';
		$class_factory_parent = $this->classPre.'Factory';
		$class_list_factory = $class.'ListFactory';
		$class_list_factory_parent = $this->classPre.'ListFactory';
		$class_multi_factory = $class.'MultiFactory';
		$class_multi_factory_parent = $this->classPre.'MutiStoreFactory';
		...
		{
			$this->writeToFile("\$this->sql = \"select `$primary_key` from `$table` where $second_key='\".my_addslashes(\$$second_key).\"'\";", 3);
		}
	...
	private function generateUpdate($columns, $table) {
		...
		foreach ($keys as $key) {
			...
			{
				$key_str = "`$key`='\".my_addslashes(\$this->".$key.").\"'";
			}
			else if($columns[$key]['type'] == 'real')
			{
		...
		if($primary_key)
		{
			...
			{
				$this->writeToFile("where `$primary_key`='\".my_addslashes(\$this->$primary_key).\"'\";", 3);
			}
	...
	private function generateConstruct($columns, $table, $primary_key) {
		...
		if($primary_key)
		{
			...
			{
				$this->writeToFile("where `$primary_key`='\".my_addslashes(\$$primary_key).\"'\";", 3);
			}
			else if($columns[$primary_key]['type'] == 'string')
			{
	...
	private function generateMultiConstruct($columns, $table, $primary_key) {
		...
		{
			$this->writeToFile("\$this->sql = \"select \$fields from $table where `$primary_key`='\".my_addslashes(\$$primary_key).\"'\";", 3);
		}
		else if($columns[$primary_key]['type'] == 'string')
		{
	...
	private function generateRetrive($columns, $class) {
		...
		$this->writeToFile("\$records = query_sql_backend(\$this->sql);", 2);
	...
	public function getTableColumns($table) {
		...
		for ($i = 0; $i < $count; $i++) {
			...
			$is_primary_key = preg_match('/primary_key/', $flags);
			$is_auto_increment = preg_match('/auto_increment/', $flags);
...
'database' => 'tb_pay_test',
'host' => 'tocartest.mysql.rds.aliyuncs.com:3306',
'parentClass' => 'kxObject',
'password' => 'ncbdtocarps',
'tables' => array(),
'user' => 'tocar',
'class_pre' => 'kx',
